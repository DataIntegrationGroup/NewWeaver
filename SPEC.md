...
| T2 | . | fix default map view: fit-to-data-extent across enabled layers OR arrival-location; >1 representative layer visible, no misleading single cluster | V5,I.map |
| T3 | . | location search on Map: address/place via client geocoder (US Census, C7); fly+drop marker; results panel "what we have / what we don't"; query OGC/ArcGIS collections by bbox (no server nearest — compute nearest client-side from bbox results). well-ID = address-only; optional client-side filter on loaded feature ids (monitoring_location_number/@iot.id/pod_file). doc choice in codebase-map | V3,C2,C7,I.map,I.svc |
